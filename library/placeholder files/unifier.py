...
for pj in paintjobs:
    ...
    for veh in vehicles:
        ...
        for cab_size in orig_cab_sizes:
            ...
            file = open("vehicle/truck/upgrade/paintjob/cm_%s/%s/cabin_%s.tobj" % (pj, veh, cab_size), "rb")
            ...
            os.remove("def/vehicle/truck/%s/paint_job/cm_%s_%s.sii" % (path, pj, cab_size))
            if accessories and cab_size not in tobj_cab_sizes:
                os.remove("def/vehicle/truck/%s/paint_job/accessory/cm_%s_%s.sii" % (path, pj, cab_size))

        for cab_size in tobj_cab_sizes:
            pass # make sii
            cab_pj_name = "cm_%s_%s" % (pj, cab_size)
