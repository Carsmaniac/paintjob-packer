    ...
    def verify_all_inputs(self):
        ...
        if inputs_verified:
            ...
            if len(warning_vehicles) > 0:
                ...
                messagebox.showwarning(title = "DBus Mods", message = "DBus Mods\n\nBecause of limitations of the game, the DBus bus mods use a workaround in order for paintjobs to appear on their doors. This means that paintjob mods work a little strangely, and will not affect the vehicles' doors.\n\nAny paintjobs generated by Paintjob Packer for the DBus buses will have a colour picker, which will allow you to change the colour of the doors, however you'll be unable to apply patterns/logos/text/etc to them. {}\n\n{}\n\nIn order to make a paintjob for a DBus bus mod that works properly, try replacing the texture files of an existing paintjob, instead of making a brand new one. If you choose to continue, expect some weirdness with your mod, and note that it cannot be fixed.".format(quantity_message, "\n".join(warning_vehicles)))
            self.main_screen.grid_forget()
